...
df_delta = spark.read.format('delta').load(sentiment_raw_delta_path)

# COMMAND ----------

from delta.tables import *
try:
  deltaTable = DeltaTable.forPath(spark, sentiment_raw_delta_path)
  deltaTable.alias('untappd_cognitive').merge(
  df.alias('updates'),
  'untappd_cognitive.checkin_id = updates.checkin_id') \
  .whenNotMatchedInsertAll() \
  .whenMatchedUpdateAll() \
  .execute()
except:
  print('table not found, creating it')
  df.write.format('delta').mode("append").save(sentiment_raw_delta_path)
