...
df_sentiment = spark.read\
          .format('delta')\
          .option('ignoreChanges', True)\
          .load(untappd_base_query_path+'sentiment')
df_sentiment_unique = df_sentiment.distinct()

# COMMAND ----------

write_delta_table(df_sentiment_unique, 'sentiment', zone = 'sanctioned')

# COMMAND ----------

register_delta_table('sentiment', zone = 'sanctioned')

# COMMAND ----------

df_sentiment_sentences = spark.read\
          .format('delta')\
          .option('ignoreChanges', True)\
          .load(untappd_base_query_path+'sentiment_sentences')
df_sentiment_sentences_unique = df_sentiment_sentences.distinct()

# COMMAND ----------

write_delta_table(df_sentiment_sentences, 'sentiment_sentences', zone = 'sanctioned')

# COMMAND ----------

register_delta_table('sentiment_sentences', zone = 'sanctioned')
