...
# MAGIC %md # Twitter Sentiment Classification using Vowpal Wabbit in MMLSpark
# MAGIC 
# MAGIC In this example, we show how to build a sentiment classification model using Vowpal Wabbit (VW) in MMLSpark. The data set we use to train and evaluate the model is [Sentiment140](http://help.sentiment140.com/for-students/?source=post_page---------------------------) twitter data. First, we import a few packages that we need.
...
# MAGIC %md Let's read the training data into a Spark DataFrame.
...
# MAGIC %md We can take a look at the training data and check how many samples it has. We should see that there are 1.6 million samples in the training data. There are 6 fields in the training data:
...
# MAGIC %md With the prepared training data, we can fit the model pipeline as follows.
...
# MAGIC %md ## Model Performance Evaluation
# MAGIC 
...
# MAGIC %md We only use positive and negative tweets in the test set to evaluate the model, since our model is a binary classification model trained with only positive and negative tweets.
...
# MAGIC %md You should see an ROC curve like the following after the above cell is executed. 
# MAGIC 
