...
df_beer_unique = df_beer.withColumnRenamed('beer_bid','beer_id').filter(df_beer.slug != 'double-shift-brewing-company-double-shift-fetching-spirits').distinct()
...
# MAGIC %sql
# MAGIC SELECT COUNT(beer_id), beer_id
# MAGIC FROM beer
# MAGIC GROUP BY beer_id
...
df_facts_unique_upsert = df_facts_upsert.join(df_facts_unique, df_facts_upsert.checkin_id == df_facts_unique.checkin_id, 'full_outer')\
...
df_facts_unique_sk = df_facts_unique_upsert.withColumn('sk_beers', concat(col('checkin_id'), col('beer_id'))).withColumn('sk_brewery', concat(col('checkin_id'), col('brewery_id'))).filter(df_facts_unique_upsert.checkin_id.isNotNull())
