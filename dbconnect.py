...
def flatten_df(nested_df):
    ...
    while len(stack) > 0:
        ...
        for c in df.dtypes:
          if c[1][:6] != "struct":
            try:
              flat_cols.append(col(".".join(parents + (c[0],))).alias("_".join(parents + (c[0],))))
            except TypeError:
              print(parents + (c[0],))
              print(".".join(parents))
              print(".".join(parents + (c[0],)))
              print(col(".".join(parents + (c[0],))))
              print(col(".".join(parents + (c[0],))).alias("_".join(parents + (c[0],))))
              tmp = col(".".join(parents + (c[0],))).alias("_".join(parents + (c[0],)))
              flat_cols.append(tmp)
        nested_cols = [
            c[0]
            for c in df.dtypes
            if c[1][:6] == "struct"
...
write_delta_table(df_toasts_flat_clean,'fact_toasts')

# COMMAND ----------

register_delta_table('df_toasts')
